...
bool FileManager::deleteFile(const QString& fileName) {
    QFile file(fileName);
    if (file.remove()) {
        LOG_INFO(QString("Deleted file: %1").arg(fileName));
        emit fileDeleted(fileName);
        return true;
    } else {
        LOG_ERROR(QString("Failed to delete file: %1").arg(fileName));
...
bool FileManager::copyFile(const QString& source, const QString& destination) {
    ...
    if (QFile::copy(source, destination)) {
        LOG_INFO(QString("Copied %1 to %2").arg(source).arg(destination));
        emit fileCreated(destination);
        return true;
    } else {
        LOG_ERROR(QString("Failed to copy %1 to %2").arg(source).arg(destination));
...
bool FileManager::renameFile(const QString& oldName, const QString& newName) {
    ...
    if (file.rename(newName)) {
        LOG_INFO(QString("Renamed %1 to %2").arg(oldName).arg(newName));
        emit fileModified(newName);
        return true;
    } else {
        LOG_ERROR(QString("Failed to rename %1 to %2").arg(oldName).arg(newName));
...
bool FileManager::cleanOldFiles(const QString& directory, int daysOld) {
    ...
    LOG_INFO(QString("Cleaned %1 old files from %2").arg(deletedCount).arg(directory));
    return true;
}
...
qint64 FileManager::getDirectorySize(const QString& directory) {
    ...
    while (it#include "data/include/file_manager.h"
#include "utils/include/logger.h"
#include <QDir>
#include <QFile>
#include <QDirIterator>
